...
// based on cs50 2023 trie problem
...
bool check(char *word)
{
    ...
    // Tries can't store words of length 0
    if (word_len == 0)
    {
        return false;
    }

    // Make sure the word is the same case as the nodes
    char word_lowercase[word_len + 1];
    for (int i = 0; i < word_len; i++)
    {
        word_lowercase[i] = tolower(word[i]);
    }
    word_lowercase[word_len] = '\0';
    
    // Walk down the nodes until we hit a null or get a match
    node *cursor = root;
    for (int i = 0; i < word_len; i++)
    {
         
        int letter_index = word_lowercase[i] - 'a';
        
        node *deeper_node = cursor->children[letter_index];
